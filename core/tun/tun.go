...
func Start(fd int, stack string, address, dns string) *sing_tun.Listener {
	...
	log.Warnln("[TUN] Start fd=%d stack=%s address=%s dns=%s", fd, stack, address, dns)
	...
	log.Warnln("[TUN] sing_tun started successfully address4=%v address6=%v dnsHijack=%v", prefix4, prefix6, dnsHijack)
	return listener
}
